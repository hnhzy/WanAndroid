package com.hzy.wanandroid.ui.activity.todo.todofragment;
...
public class ToDoFragment extends BaseMvpFragment<ToDoPresenter> implements ToDoContract.View {
    ...
    protected void initEventAndData() {
        ...
        mAdapter = new ToDoAdapter(getActivity(), mListToDo, mPresenter);
    ...
    public void done(int position, int subPos, int status, ResponseBean responseBean) {
        if (responseBean.getErrorCode() == 0) {
            mListToDo.get(position).get(subPos).setWhere("ToDoFragment");
    ...
    public void onMessageEvent(ToDoBean bean) {
        ...
        } else if (bean.getWhere().equals("DoneFragment")) {
            bean.setStatus(0);
            onGetData(bean);
        }
    }
